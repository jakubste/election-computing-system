...
Dla \, $p = 1$, \, $l_1 \equiv +$:
\begin{equation}
f_{\ell_pB}(1,(i_1, \dots ,i_k)) = \beta(i_1) + \beta(i_2) + \dots + \beta(ik) = f_{kB}(i_1, \dots,i_k)
\end{equation}
...
Wizualizacja dotyczy wyborów wygenerowanych z rozkładu normalnego lub stworzonych poprzez zaznaczanie 
punktów na płaszczyźnie.\\ 
Wyborcy i kandydaci są reprezentowani jako punkty na płaszczyźnie. Punkty reprezentujące wyborców
...
zarządzania wyborami, który zleca mu zadanie wizualizacji wyborów lub jego wyników. W
...
\subsection{Opis ogólny}
Moduł odpowiedzialny za generację wyborów z rozkładu normalnego, generację poprzez zaznaczanie punktów 
na płaszczyźnie oraz wczytywanie wyborów z pliku formatu \textit{.soc}. Moduł współpracuje z modułem 
zarządzania wyborami, któremu zleca po wykonaniu swoich zadań, stworzenie i wysłanie użytkownikowi
odpowiedniej strony internetowej. Moduł zapewnia wygenerowanie wyborów z rozkładu normalnego według 
wskazanych parametrów, wygenerowanie wyborow na podstawie zaznaczonych przez uzytkownika punktów oraz 
walidację danych przy wczytywaniu wyborów z pliku. Po stworzeniu wyborów moduł komunikuje się z bazą 
danych w celu utrwalenia wyborów.
...
\cline{2-3}
 & klasa
\code{ElectionPaintView} & pobranie punktów zaznaczonych przez użytkownika, odpowiednie sformatowanie 
danych, przekazanie ich do bazy \\
\hline
...
rozkładu normalnego \\
\cline{2-3}
 & \code{election\_paint\_data.html} & strona z interfejsem do zaznaczania kandydatów i wyborców na 
 płaszczyźnie\\
\cline{2-3}
...
\label{subsec:wyswietlanie_informacji_szczegolowych_dla_danych_wyborow}
\begin{enumerate}
...
\item Jeżeli wybory zostały wygenerowane z rozkładu normalnego lub poprzez zaznaczenie punktów na 
płaszczyźnie, widok przekierowuje działanie programu do modułu wizualizacji wyborów i wyników wyborów 
(widok \code{ElectionChartView}). Jeżeli wybory nie zostały wygenerowane z w jeden z dwóch 
wymienionych wyżej sposobów, następuje przejście do kroku nr $6$
...
wyborów}
\begin{enumerate}
\item Użytkownik klikając na odpowiednią ikonę przy danym wyniku wyborów na stronie ze szczegółowymi
...
\item Jeżeli wybory zostały wygenerowane z rozkładu normalnego lub poprzez zaznaczenie punktów na 
płaszczyźnie, widok przekierowuje działanie programu do modułu wizualizacji wyborów i wyników wyborów 
(widok \code{ResultChartView}). Jeżeli wybory nie zostały wygenerowane w jeden z dwóch wymienionych 
wyżej sposobów, następuje przejście do kroku nr $6$.
\clearpage
\item Widok pobiera z bazy danych obiekty będące punktami \textit{2D} reprezentującymi
...
określenia parametrów wyborów oraz rozkładu normalnego.
...
zostają zapisane do bazy. Następnie program przechodzi do 2. punktu ścieżki
\textit{\nameref{subsec:wyswietlanie_informacji_szczegolowych_dla_danych_wyborow}}.
\end{enumerate}

\subsection{Tworzenie wyborów poprzez zaznaczenie punktów na płaszczyźnie}
\begin{enumerate}
\item Użytkownik klikając na przycisk \textit{Draw chart} na stronie ze szczegółowymi informacjami o danych wyborach, wysyła za pomocą przeglądarki internetowej żądanie stworzenia wyborów w edytorze graficznym.
\item Moduł URL Resolver kieruje żądanie do modułu generacji i wczytywania wyborów \mbox{(widok
\code{ElectionPaintView})}.
\item Widok przekierowuje sterowanie do \code{election\_paint\_data.html} - szablonu z edytorem graficznym.
\item Użytkownik zaznacza kandydatów i wyborców, tworzone są listy współrzędnych kandydatów \mbox{i wyborców}. 
\item Użytkownik klika przycisk \textit{Save and proceed}. Listy współrzędnych kandydatów i wyborców 
zostają przesłane do widoku \textit{ElectionPaintView}
\item Widok \code{ElectionPaintView} zapisuje dane do bazy z wykorzystaniem instancji klasy 
\code{ElectionPaintLoader} przekazując jej listy współrzędnych kandydatów i wyborców. Następnie 
program przechodzi do 2. punktu ścieżki 
\textit{\nameref{subsec:wyswietlanie_informacji_szczegolowych_dla_danych_wyborow}}.
\end{enumerate}
...
\section{Diagram ERD}
\begin{center}
\centerline{\includegraphics[scale=0.25]{pics/ECS_ERD.png}}
...
umieszczone są w rozdziale \textit{\nameref{cha:dziedzina_problemu}}. Ogólne zadowolenie wyborców z 
komitetu jest obliczane jako suma pojedynczych zadowoleń wszystkich wyborców. Algorytm działa w
...
Do zbadania poprawności działania innych algorytmów zaimplementowany został bardzo prosty algorytm typu \textit{brute-force} iterujący po wszystkich możliwych kombinacjach komitetów i wyszukujący komitet o najwyższym wyniku.
