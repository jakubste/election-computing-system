...
\subsection{Metoda k-Bordy}
\label{subsec:metoda_k_bordy}
...
\subsection{Uogólnienie - system $\ell_p$-Bordy}
...
Wówczas, w uogólnionej wersji metody k-Bordy, funkcja zadowolenia $f_{kB}$ (1.2) zostaje uzależniona również od parametru $p$ ze wzoru (1.3). Norma liczona jest z wyników według Bordy, $\beta(i)$ (1.1).\\Wzór uogólniony funkcji zadowolenia przyjmuje zatem postać:
\begin{equation}
f_{\ell_pB}(p,(i_1,\dots,i_k))=\sqrt[p]{[(i_1)]^p+[(i_2)]^p+...+[(i_k)]^p}
\end{equation}

Systemy  k-Bordy i Cahmberlin’a-Courant’a są szczególnymi przypadkami zdefiniowanego powyżej systemu $\ell_p$-Bordy:\\
Dla \, $p = 1$, \, $l_1$:
\begin{equation}
...
\cline{2-3}
 & klasa \textit{RegistrationForm} & rejestracja \\
\hline
\end{tabular}
}
...
\textbf{Typ komponentu} & \textbf{Komponenty} & \textbf{Wykorzystanie} \\
\hline
widoki & klasa \code{ScatterChartMixin} & pobranie danych potrzebnych do
...
\cline{2-3}
& klasa \code{AlgorithmsComparisonChartView} & zestawia dla danych wyborów czasy wykonania obliczeń  przez rózne algorytmy dla róznych wartości parametru $p$\\
\hline
...
widoków) znajdują się w pliku \code{ecs.elections.urls.py}.
...
pomocą funkcji: $$f_{{\ell_p}-Borda}(i_1, i_2,\dots, i_k) = \sqrt[p]{[\beta(i_1)]^p+[\beta(i_2)]^p+\dots+[\beta(i_k)]^p}$$ Funkcja bazuje na normie $\ell_p$ oraz punktacji $k-Bordy$. Dokładne informacje na temat funkcji
...
wyborców. W regule \textit{Chamberlin-Courant’a} położony jest nacisk na regułę reprezentacji proporcjonalnej. Każdy wyborca reprezentowany jest przez jednego, wyraźnego kandydata i
...
\subsection{Funkcja zadowolenia}
Aktualny poziom zadowolenia wyborców na danym etapie algorytmu określany jest za
pomocą funkcji satysfakcji \textit{Chamberlin-Courant’a}:
\begin{equation}
...
Algorytmy genetyczne polegają na przedstawieniu róznych wynikó problemu 
...
Ponieważ algorytm zachłanny według zasady \textit{Chamberlin'a-Courant’a} przystosowany jest do
...
\textit{Chamberlin'a-Courant’a} staje się nie gorszy od swojego konkurenta pod względem jakości
wyniku. Dla wartości parametru $p$ większych od tej wartości granicznej, algorytm zachłanny według zasady
\textit{Chamberlin'a-Courant’a} jest co najmniej tak samo dobry jak algorytm zachłanny zależny od
...
przewyższając tym samym algorytm według zasady \textit{Chamberlin'a-Courant’a}, który zawsze wybierze drugiego
